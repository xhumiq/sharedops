...
PROMPT='
%{$fg[gray]%}%m%{$reset_color%}%{$fg[cyan]%}%~%{$reset_color%} %{$fg[gray]%}%t %{$reset_color%}$(git_prompt_short_sha)$(git_prompt_info) [%{$fg[red]%}${CURRENT_PROJ}%{$reset_color%}_${AWS_PROFILE}/${PROJ_STAGE} @${AWS_REGION}] $(prompt_docker_machine_name)
